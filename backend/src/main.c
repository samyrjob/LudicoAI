...
if (!g_audio) {
    ...
    #ifdef _WIN32
    // On Windows, WASAPI is not implemented yet - continue without audio
    fprintf(stderr, "[Main] WARNING: Audio capture not available on Windows\n");
    fprintf(stderr, "[Main] Backend will run but won't capture audio\n");
    ipc_send_status("Audio capture unavailable on Windows - backend running in limited mode");
    #else
    // On other platforms, audio is required
    ipc_send_error("Failed to initialize audio capture");
    whisper_engine_cleanup(g_whisper);
    if (g_translator) translation_cleanup(g_translator);
    ipc_cleanup();
    return 1;
    #endif
}

/* Start audio capture (only if available) */
if (g_audio) {
    if (!audio_start(g_audio)) {
        fprintf(stderr, "[Main] Failed to start audio: %s\n", audio_get_error());
        ipc_send_error("Failed to start audio capture");
    ...
    ipc_send_status("Running in no-audio mode (audio capture not available)...");
    fprintf(stderr, "[Main] Running in no-audio mode (press Ctrl+C to stop)\n");
}
