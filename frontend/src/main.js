...
function startBackend() {
    ...
    if (isWindows) {
        // Windows: Use native Windows build
        backendPath = path.join(__dirname, '../../build-windows/Release/visualia.exe');
        modelPath = path.join(__dirname, '../../models', modelFile);
        ...
        console.log('[Electron] Using Windows native build');
    } else if (isLinux) {
        // Linux: Use Linux build
        backendPath = path.join(__dirname, '../../build-linux/visualia');
        modelPath = path.join(__dirname, '../../models', modelFile);
        ...
        console.log('[Electron] Using Linux native build');
    } else if (isMac) {
        // macOS: Use macOS build
        backendPath = path.join(__dirname, '../../build/visualia');
        modelPath = path.join(__dirname, '../../models', modelFile);
        ...
        console.log('[Electron] Using macOS native build');
    } else {
        console.error('[Electron] Unsupported platform:', process.platform);
        return;
    }

    console.log('[Electron] Starting backend:', backendPath);
    console.log('[Electron] Arguments:', args.join(' '));
...
function stopBackend() {
    if (backendProcess) {
        ...
        // Use appropriate signal based on platform
        if (process.platform === 'win32') {
            backendProcess.kill('SIGTERM');  // Windows
        } else {
            backendProcess.kill('SIGTERM');  // Unix-like systems
        }
